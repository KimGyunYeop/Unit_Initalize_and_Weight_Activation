...
GPU="${GPU:-0}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/rebuttal_runs/generation_t5}"
RESULT_TAG="${RESULT_TAG:-t5_rebuttal}"
...
run_one() {
  ...
  local preset="$2"
  local seed="$3"
  local existing_metrics_glob="$OUTPUT_ROOT/t5/$task/$preset/${RESULT_TAG}_t5_${task}_*/seed_${seed}/metrics.json"
  ...
  local cmd=(
    "$PYTHON_BIN" test_generation.py
    --result_path "${RESULT_TAG}_t5"
...
echo "[done] generation rebuttal suite finished"
